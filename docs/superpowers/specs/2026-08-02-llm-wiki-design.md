...
`raw/`는 사용자가 소유하는 변경 불가능한 원본 계층이다. 논문 PDF, web page snapshot, YouTube metadata와 transcript, supplementary material, 사용자가 제공한 원문 메모, 이미지와 표 등을 보존한다. LLM은 사용자가 대화에서 보존을 요청한 원문을 새 파일로 생성할 수 있지만, 생성되거나 사용자가 추가한 기존 파일은 수정하거나 덮어쓰지 않는다.

### 2.2 Wiki

`wiki/`는 LLM이 소유하고 관리하는 지식 계층이다. 논문·web·YouTube별 근거 노트, 개념 종합, 사용자 생각의 구조화본, 열린 질문, 색인 및 작업 로그를 포함한다. 새 원본이 추가되거나 중요한 질문이 해결될 때 기존 페이지를 함께 갱신한다.
...
`inbox/`는 두지 않는다. 사용자는 새 논문을 `raw/sources/papers/`에 두거나 대화에서 web·YouTube URL을 전달한다. URL에서 확보한 snapshot과 transcript는 각각 `raw/sources/web/`과 `raw/sources/youtube/`에 저장한다. 사용자 원문 메모는 `raw/notes/`, 첨부물은 `raw/assets/`에 보존한다. 처리 여부와 학습 상태는 원본 위치가 아니라 Wiki Source 페이지의 metadata로 관리한다.
...
Raw 파일은 원본 노드로 취급하되 LLM 생성 metadata를 원본 파일에 삽입하지 않는다. 대응하는 Source 또는 Thought 페이지에서 원본 경로를 참조한다.
...
페이지 유형에 따라 raw source, `source_type`, canonical URL, DOI, arXiv ID, 관계, 학습 상태 등을 추가한다. `status`는 페이지 유형별 지식·학습 상태를 나타내고, Source의 원본 처리 성공 여부는 별도의 `ingestion_status` 필드에서 `complete` 또는 `partial`로 관리한다. 날짜는 ISO 8601 형식을 사용한다. 빈 배열은 필드가 누락된 것과 구분한다.
...
Source는 자료가 실제로 무엇을 말했는지 추적하는 근거 페이지다. 모든 Source는 제목, 작성자 또는 채널, 발행일, 접근일, 원본 위치, 학습 상태, 명시적 주장, LLM 사전 분석, 관련 Concept·Thought·Question을 포함한다.

Paper subtype은 DOI 또는 arXiv ID, research question, method, data and experimental setup, results, limitations를 추가한다. 인용에는 가능한 경우 page, section, figure, table 또는 equation 위치를 포함한다.

Web subtype은 canonical URL, page title, site, author, published date, accessed date, local snapshot 및 page revision 정보를 추가한다. 인용에는 section heading과 local snapshot 위치를 포함한다.

YouTube subtype은 canonical URL, video ID, title, channel, published date, duration, accessed date, transcript language 및 local transcript를 추가한다. 인용에는 가능한 경우 timestamp 또는 chapter를 포함한다.

`LLM 사전 분석`은 읽기 안내일 뿐 확정된 사용자 지식이 아니다. Web page와 video의 신뢰도는 형식이 아니라 작성자, 근거, 최신성 및 다른 자료와의 교차 검증으로 평가한다.
...
2. Source 페이지를 만들거나 기존 페이지에 새 revision을 연결한다.
...
사용자가 Web 또는 YouTube URL을 전달하면 다음 순서로 처리한다.

1. URL을 정규화하고 canonical URL 또는 YouTube video ID로 중복을 확인한다.
2. 제목, 작성자·channel, 발행일, 접근일과 같은 metadata를 확보한다.
3. Web page는 읽을 수 있는 본문을 Markdown snapshot으로 저장하고, 지식에 필요한 로컬 이미지가 있으면 `raw/assets/`에 보존한다.
4. YouTube는 제공되는 transcript 또는 caption과 chapter 정보를 저장한다. transcript에 timestamp 정보가 있으면 함께 보존한다.
5. 확보한 원문을 `raw/sources/web/` 또는 `raw/sources/youtube/`에 immutable capture로 저장한다.
6. 대응하는 Wiki Source 페이지를 만들고 `unread` 또는 `learning`으로 등록한다.
7. `index.md`와 `log.md`에 capture 범위와 실패 항목을 기록한다.

URL만 남겨두지 않고 로컬 capture를 보존하는 이유는 page 수정, video 삭제, transcript 변경 및 링크 단절 이후에도 당시 근거를 재현하기 위해서다. capture에는 원본 URL과 접근일을 반드시 기록한다.

YouTube transcript만 확보한 경우 `coverage: transcript-only`로 표시한다. LLM은 직접 확인하지 않은 화면의 code, diagram, demo 또는 visual result를 봤다고 주장하지 않는다. 영상과 transcript를 모두 확인했을 때만 `coverage: audiovisual`을 사용할 수 있다.

로그인, paywall, robots policy, 동적 page, transcript 부재 또는 지역 제한 때문에 원문을 충분히 확보하지 못하면 `ingestion_status: partial`로 기록한다. URL, 확보한 metadata, 실패 원인과 사용자에게 필요한 후속 조치를 남기고 Knowledge ingest를 완료하지 않는다. 사용자는 Markdown clip, PDF, transcript 또는 video file을 직접 제공해 capture를 보완할 수 있다.

URL만 전달한 경우 기본 동작은 Register ingest다. 사용자가 함께 학습을 요청하면 capture 이후 Study/query로 이어가고, 자신의 메모를 제공하면 원문 보존 후 Thought 또는 Question으로 연결한다. 사용자가 해당 자료를 이해했으며 정식 반영하라고 명시한 경우에만 Knowledge ingest까지 진행한다.
...
### 9.4 Knowledge ingest

사용자가 이해했다고 선언하고 정식 반영을 요청한 경우 수행한다.

1. Source 상태를 `understood`로 전환한다.
2. 자료의 주장, 방법, 결과, 한계와 인용 위치를 검증한다.
...
8. 검증 완료 후 Source 상태를 `integrated`로 바꾼다.
...
- 장기간 `unread`, `learning` 또는 `revisit` 상태인 Source
- 이해 선언 후 통합되지 않은 자료
- 해결 조건이 불명확한 Question
- URL Source의 local capture 또는 accessed date 누락
- YouTube 인용의 timestamp 누락과 transcript-only 범위 위반
...
PDF 페이지, Web 본문, 수식, 표, 이미지, YouTube transcript 또는 supplementary material을 읽지 못하면 Source의 `ingestion_status`를 `partial`로 표시한다. 확인하지 못한 범위와 필요한 후속 자료를 기록하고 Knowledge ingest를 완료하지 않는다.

### 10.3 중복과 revision

Paper는 DOI, arXiv ID, 제목 및 저자로 중복을 확인한다. Web은 canonical URL, YouTube는 video ID로 중복을 확인한다. 같은 capture 또는 version이면 기존 Source에 연결한다. 새 revision이나 변경된 page snapshot이면 원본을 별도로 보존하고 변경 내용을 비교한 뒤 기존 지식에 영향을 주는 부분만 갱신한다.

### 10.4 원자적 변경과 Git

Source 하나의 등록, 하나의 Knowledge ingest, 저장하기로 한 query 결과, 또는 lint 수정 하나를 하나의 논리적 변경 단위로 다룬다. 관련 페이지, index 및 log를 함께 검증한 뒤 작업 유형이 드러나는 Git commit으로 보존한다.
...
- 사용자 안내인 `README.md`
- Concept, 공통 Source, Paper, Web, YouTube, Thought, Question 템플릿
...
- 논문, Web snapshot, YouTube transcript와 사용자 원문을 수정 없이 보존할 수 있다.
- 읽지 않은 자료를 확정 지식과 분리해 등록할 수 있다.
- Web 또는 YouTube URL 하나로 재현 가능한 local capture와 Source 페이지를 등록할 수 있다.
- 학습 중 질문과 설명을 기존 지식과 연결할 수 있다.
- 사용자 선언 후 Source를 Concept graph에 통합할 수 있다.
- Source의 주장, 사용자 생각 및 LLM 분석이 명확히 구분된다.
- 반증된 가설과 잘못 구성된 질문의 이력이 보존된다.
- index에서 전체 위키를 탐색할 수 있다.
- Obsidian Graph View에서 Paper·Web·YouTube Source, Concept, Thought 및 Question 관계를 확인할 수 있다.
